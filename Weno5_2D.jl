...
const Nx = 16 + 2*NBnd	# number of cells including boundaries
const Ny = 4  + 2*NBnd	# number of cells including boundaries
...
function WENO5_2D()
    ...
    cmap = ColMap(42)  # you could choose other maps of course
...
dt = 1e-8
        @printf("\n%d : t = %g, dt = %g, vmax = %26.20g, %26.20g \n",step,t,dt,vxmax,vymax)
...
flag = false
    ...
    q1_E = q0_E - 1/2 * dt/dy * dFy0_E - 1/2 * dt/dx * dFx0_E      # rk4 step 1 for E state
    ...
    q2_E = q0_E  - 1/2 * dt/dy * dFy1_E - 1/2 * dt/dx * dFx1_E
...
#println("$pt $vB")
#for m=1:7
#@printf("%d %23.16f %23.16f %23.16f\n",
#        m,F[i,j,m], q[i,j,m], u[i,j,m])
#end
#PQ(q,6)
#@assert false
#end
...
function initial_conditions() # Shock Cloud interaction
	...
	for j = 1:Ny
		for i = 1:Nx
            ...
            x = (i - 1.0 + 0.5) * dx - NBnd*dx
            y = (j - 1.0 + 0.5) * dy - NBnd*dy
...
function PQRMBND(q,c)

    println("PQRMBND c=$c")

    for i=NBnd+1:size(q,1)-NBnd
        @printf "%3i" i-NBnd
        
        for j=NBnd+1:size(q,2)-NBnd
            @printf("%25.20f ", q[i,j,c])
        end
        println(" ")
    end
...
function PSRMBND(q)
    ...
    for i=1+NBnd:size(q,1)-NBnd
        @printf("%2g ", i-NBnd)
        
        for j=1+NBnd:size(q,2)-NBnd
            @printf("%21.16f", q[i,j])
        end
        println(" ")
    end
